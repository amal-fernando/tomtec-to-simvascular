...
import tetgen
import os
from boundary import extract_surface_region_from_old
...
normalplot(v0, f0, 1, os.path.join(radice_dataset[:-1], "plots", "normalplot_coarse.obj"),
           show=False)  # plot delle normali alla mesh rada \decommentare
...
if flag:
    ...
    normalplot(vert, fac, 0, os.path.join(radice_dataset[:-1], "plots", "normalplot_fine.obj"),
               show=False)  # Plot the normals of the remeshed mesh
else:
...
tet.tetrahedralize(order=1,
                   switches=tetgen_options)  # "pq1.2a0.333" or tet.tetrahedralize(order=1, switches=tetgen_options)
